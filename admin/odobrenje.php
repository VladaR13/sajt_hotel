...
if (isset($_POST['odobri'])) {
    ...
    $stmt = $conn->prepare("UPDATE rezervacija SET status='odobreno' WHERE rezervacija_id=?");
...
if (isset($_POST['odbij'])) {
    $rez_id = intval($_POST['rezervacija_id']);
    $stmt = $conn->prepare("UPDATE rezervacija SET status='odbijeno' WHERE rezervacija_id=?");
    $stmt->bind_param("i", $rez_id);
    $stmt->execute();
    $stmt->close();
}
...
<div class="container mx-auto mt-10">
    ...
    <table class="min-w-full bg-white rounded shadow overflow-hidden">
        ...
        <tbody>
            ...
            <tr class="<?= $row['status'] === 'na_cekanju' ? 'bg-yellow-100' : ($row['status']==='odobreno' ? 'bg-green-100' : 'bg-red-100') ?>">
